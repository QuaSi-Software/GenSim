"""
Zone sensible energy balance plot (signed), incl. NET residual and NET% vs Σ|E|.
...
def create_zone_balance_plot(df: pd.DataFrame, include_abs_sum_bar: bool = False) -> None:
    ...
    title = f"Zone sensible balance (signed) | NET={net:,.2f} kWh ({pct_net_vs_abs:,.2f}% of Σ|E|)"
...
def main() -> None:
    ...
    parser.add_argument(
        "csv",
        nargs="?",
        default=r"F:\Repos\OrgGenSim\Output\run\013_results\report_variables_ZoneTimestep-Sum.csv",
